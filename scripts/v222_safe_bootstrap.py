...
BOOTSTRAP_WORKTREE_PATH = (
    PROJECT_ROOT + "/scripts/v222_safe_bootstrap.py"
)

RUNNER_PATH = PROJECT_ROOT + "/scripts/probe_sparse_multiview_g0.py"
...
def _validate_early_runtime(target: dict[str, str], target_arguments: tuple[str, ...]) -> dict[str, str]:
    ...
    if target["mode"] == "direct":
        ...
        if not (
            ...
            == _path_key(os.path.join(bootstrap_root, "v222_safe_bootstrap.py"))
...
def _bootstrap_main(raw_arguments: list[str]) -> int:
    ...
    if target["mode"] == "module":
        ...
        if os.listdir(bootstrap_root) != ["v222_safe_bootstrap.py"]:
