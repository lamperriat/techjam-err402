"""Sterile, source-only launcher for the v2.23 OOV chargram bridge probe.
...
PROJECT_ROOT = "D:/tiktok/techjam-v2-23-oov-chargram-bridge"
RUNTIME_BASE = "D:/tiktok/.v223_runtime"
...
CRITICAL_BINARY_ROWS = (
    (
        "D:/450/conda/envs/tiktok/python311.dll",
        6_193_152,
        "08701864dea4e08c077c1c5bc6cb208d5628dba9473a318fa6ce3796a86806c5",
    ),
    (
        "D:/450/conda/envs/tiktok/DLLs/_sqlite3.pyd",
        99_328,
        "67858a7dcbce3abef73328276c19dc06abe2515ab01d7cbc7fc7ad7bd3e2114c",
    ),
    (
        "D:/450/conda/envs/tiktok/Library/bin/sqlite3.dll",
        3_239_936,
        "9519340d2ede13b05cd889605e3a46cc6bde702f266061266e22c18a0951a04e",
    ),
)
BOOTSTRAP_WORKTREE_PATH = (
    PROJECT_ROOT + "/scripts/v223_safe_bootstrap.py"
)

RUNNER_PATH = PROJECT_ROOT + "/scripts/probe_oov_chargram_bridge_g0.py"
...
ATTESTATION_ATTRIBUTE = "_techjam_v223_bootstrap_attestation"
...
_LOCAL_SOURCE_ROWS = {
    ...
    "starter.oov_chargram_bridge_g0": (
        ...
        "f7bdfccfd2d3111b735757c03cdf2d96ec5f88f3",
        False,
    ),
}

_LOCAL_PREFIXES = ("evaluator", "scripts", "starter")
_DENIED_EXPERIMENT_PREFIXES = (
    "small_ranker_v2_19_",
    "small_ranker_v2_20_",
...
_DENIED_RUNTIME_COMPONENTS = frozenset(
    {
        ".v219_runtime", ".v220_runtime", ".v220b_runtime", ".v221_runtime",
        ".v222_runtime", ".v222b_runtime",
    }
)
_DENIED_WORKTREE_PREFIXES = (
...
def _is_old_experiment_denied(value: object) -> bool:
    components = _path_components(value)
    if any(component in _DENIED_RUNTIME_COMPONENTS for component in components):
        return True
    if any(
        component.startswith(prefix)
        for component in components
        for prefix in _DENIED_WORKTREE_PREFIXES
    ):
        return True
    for index in range(max(0, len(components) - 2)):
        ...
        if any(candidate.startswith(prefix) for prefix in _DENIED_EXPERIMENT_PREFIXES):
            return True
    return False
...
def _guard_path(value: object, *, listing: bool = False) -> None:
    if _is_old_experiment_denied(value) or (listing and _is_fast_track_listing(value)):
        raise PermissionError("OLD_EXPERIMENT_NAMESPACE_DENIED")
...
def _validate_early_runtime(target: dict[str, str], target_arguments: tuple[str, ...]) -> dict[str, str]:
    ...
    if target["mode"] == "module":
        ...
        required_environment.add("PYTHONPATH")
    elif "PYTHONPATH" in os.environ:
        raise BootstrapError("DIRECT_PYTHONPATH")
    folded_environment: dict[str, str] = {}
    for key, value in os.environ.items():
        upper = key.upper()
        if upper in folded_environment:
            raise BootstrapError("ENVIRONMENT_CASE_ALIAS")
        folded_environment[upper] = value
    ...
    if any(os.environ.get(key) != value for key, value in allowed_python.items()):
        raise BootstrapError("PYTHON_ENVIRONMENT")
    if set(folded_environment) != required_environment:
        raise BootstrapError("ENVIRONMENT_KEY_SET")
    expected_values = {
        "CUDA_VISIBLE_DEVICES": "",
        "HF_HUB_OFFLINE": "1",
        "MKL_NUM_THREADS": "1",
    ...
    if any(folded_environment.get(key) != value for key, value in expected_values.items()):
        raise BootstrapError("ENVIRONMENT_VALUE_SET")
    system_root = folded_environment["SYSTEMROOT"].rstrip("\\/")
    if _path_key(folded_environment["PATH"]) != _path_key(system_root + "/System32"):
        raise BootstrapError("PATH_ENVIRONMENT")
    if _path_key(folded_environment["TEMP"]) != _path_key(folded_environment["TMP"]):
        raise BootstrapError("TEMP_ENVIRONMENT")
    temp_key = _path_key(folded_environment["TEMP"])
    if not (
        temp_key.startswith(_path_key(RUNTIME_BASE) + "\\v223-")
    ...
    if target["mode"] == "direct":
        ...
        if not (
            ...
            == _path_key(os.path.join(bootstrap_root, "v223_safe_bootstrap.py"))
...
def _validate_executable() -> None:
    ...
    for path_text, expected_bytes, expected_sha256 in CRITICAL_BINARY_ROWS:
        payload, _identity = _read_descriptor(path_text)
        if len(payload) != expected_bytes or hashlib.sha256(payload).hexdigest() != expected_sha256:
            raise BootstrapError("PYTHON_CRITICAL_BINARY")
...
def _validate_temp() -> tuple[str, tuple[int, ...]]:
    raw = os.environ.get("TEMP")
    if not isinstance(raw, str) or not raw or _path_key(raw) != _path_key(os.environ.get("TMP", "")):
        raise BootstrapError("TEMP_LOCATION")
    observed = _require_plain_ancestry(raw, directory=True)
    if os.listdir(raw):
        raise BootstrapError("TEMP_NOT_EMPTY")
    return raw.replace("\\", "/"), _signature(observed)
...
def _bootstrap_main(raw_arguments: list[str]) -> int:
    ...
    temp_path, temp_signature = _validate_temp()

    if target["mode"] == "module":
        bootstrap_root = startup["bootstrap_root"]
        if os.listdir(bootstrap_root) != ["v223_safe_bootstrap.py"]:
    ...
    _revalidate_empty_directory(temp_path, temp_signature)
