...
$RunnerRelative = "scripts/probe_sparse_multiview_g0.py"
$WorkerRelative = "scripts/sparse_multiview_g0_worker.py"
$UnionRelative = "starter/sparse_multiview_g0.py"
$PowerShellRelative = "scripts/run_v222_preflight.ps1"
$TestRelative = "tests/test_sparse_multiview_g0.py"
$ImplementationPaths = @(
    $RunnerRelative,
    $WorkerRelative,
    $BootstrapRelative,
...
function Get-GitControlCheckpoint {
    ...
    $configText = Convert-Utf8Strict -Bytes $configRaw
    if ($configText -cmatch "(?mi)^\s*\[(?:include|includeif)\b") {
        Throw-Code "GIT_CONFIG_INCLUDE_DENIED"
    }
    return ($parts -join "`n")
}
...
function Get-DirectoryStableIdentity {
    param([Parameter(Mandatory = $true)][string]$Path)
    if ($null -eq ("V222DirectoryIdentity" -as [type])) {
        $source = @'
using System;
...
public static class V222DirectoryIdentity {
...
'@
    ...
    try { $identity = [V222DirectoryIdentity]::Read([System.IO.Path]::GetFullPath($Path)) } catch { Throw-Code "RUNTIME_IDENTITY_BACKEND" }
...
function Remove-OwnedFreshRuntime {
    ...
    if ([System.IO.Directory]::Exists($moduleRoot)) {
        ...
        if ($moduleChildren.Count -ne 1 -or $moduleChildren[0].Name -cne "v222_safe_bootstrap.py" -or -not ($moduleChildren[0] -is [System.IO.FileInfo])) {
...
function Invoke-PreclaimEntrypointCheck {
    ...
    try {
        ...
        if ($InvocationMode -eq "direct") {
            ...
            Assert-PlainExistingPath -Path $moduleRoot -Kind Directory | Out-Null
            $modulePath = Join-Path $moduleRoot "v222_safe_bootstrap.py"
            ...
            $arguments += @("-m", "v222_safe_bootstrap")
        }
        $arguments += @(
            "--mode", $InvocationMode,
            ...
            "--",
            "--entrypoint-self-check", "--require-module", "starter.sparse_multiview_g0"
        )
        ...
        if (
            ...
            [string]$outer.target_receipt.required_module -cne "starter.sparse_multiview_g0" -or
...
function Invoke-PreclaimEntrypointChecks {
    ...
    $targets = @(
        [pscustomobject]@{ Path = $RunnerPath; Module = "scripts.probe_sparse_multiview_g0"; Relative = $RunnerRelative },
        [pscustomobject]@{ Path = $WorkerPath; Module = "scripts.sparse_multiview_g0_worker"; Relative = $WorkerRelative }
    )
...
function Invoke-PreclaimPrerequisiteCheck {
    ...
    try {
        ...
        $arguments = @(
            ...
            "--target-module", "scripts.probe_sparse_multiview_g0",
...
function Assert-FormalCacheSnapshot {
    ...
    Assert-ExactNames -Value $Snapshot -Expected @(
        "clears", "closed", "enabled", "fts_route", "mask_decision", "product_view"
    )
    Assert-ExactBoolean -Value $Snapshot.enabled -Expected $true -Code $Code
    Assert-ExactBoolean -Value $Snapshot.closed -Expected $AfterClose -Code $Code
    Assert-ExactInteger -Value $Snapshot.clears -Expected $(if ($AfterClose) { 1L } else { 0L }) -Code $Code
    $capacities = [ordered]@{ fts_route = 512L; product_view = 4096L; mask_decision = 16384L }
    foreach ($layerName in $capacities.Keys) {
        ...
        Assert-ExactNames -Value $layer -Expected @(
            "avoided_operations", "capacity", "evictions", "hits", "lookups", "misses", "size"
        )
        foreach ($field in @("avoided_operations", "capacity", "evictions", "hits", "lookups", "misses", "size")) {
        ...
        if (
            ...
            [int64]$layer.lookups -ne ([int64]$layer.hits + [int64]$layer.misses) -or
            [int64]$layer.avoided_operations -ne [int64]$layer.hits -or
...
function Assert-FormalCachePair {
    ...
    foreach ($layerName in @("fts_route", "product_view", "mask_decision")) {
        $before = $Cache.before_close.PSObject.Properties[$layerName].Value
        $after = $Cache.after_close.PSObject.Properties[$layerName].Value
        foreach ($field in @("avoided_operations", "capacity", "evictions", "hits", "lookups", "misses")) {
...
function Assert-FormalCompactWorker {
    ...
    if ([string]$Worker.device -cne "CPU" -or [string]$Worker.provider -cne "SQLite FTS5 + CPython") { Throw-Code $Code }
    ...
    Assert-ExactNames -Value $Worker.activation -Expected @(
        "category_only_route_activation_records", "exact_active_route_activation_records",
        "full_positive_route_activation_records", "legacy_route_executions",
        "multiroute_support_candidate_cells", "title_store_exact_route_activation_records",
        "union_expansion_records", "union_novel_candidate_cells"
    )
    ...
    foreach ($route in @("full_positive", "exact_active", "category_only", "title_store_exact")) {
        if ([int64]$Worker.activation.PSObject.Properties["${route}_route_activation_records"].Value -gt $records) { Throw-Code $Code }
    }
    if ([int64]$Worker.activation.union_expansion_records -gt $records) { Throw-Code $Code }
    ...
    $null = Assert-FormalLatencySummary -Summary $Worker.latency.extra_routes_fusion_and_mask -ExpectedCount $records -MaximumP95 100.0 -Code $Code
    ...
    Assert-ExactNames -Value $Worker.latency.fts_routes -Expected @("category_only", "exact_active", "full_positive", "title_store_exact")
    $routeExecutionSum = 0L
    foreach ($route in @("full_positive", "exact_active", "category_only", "title_store_exact")) {
        $routeLatency = $Worker.latency.fts_routes.PSObject.Properties[$route].Value
        Assert-ExactNames -Value $routeLatency -Expected @("count", "maximum_milliseconds", "p50_milliseconds", "p95_milliseconds")
        $activation = [int64]$Worker.activation.PSObject.Properties["${route}_route_activation_records"].Value
        $routeCountValue = $routeLatency.count
        if (-not ($routeCountValue -is [int] -or $routeCountValue -is [long])) { Throw-Code $Code }
        $routeCount = [int64]$routeCountValue
        if ($routeCount -lt 0 -or $routeCount -gt $activation -or (-not $ExpectedCache -and $routeCount -ne $activation)) { Throw-Code $Code }
        $null = Assert-FormalLatencySummary -Summary $routeLatency -ExpectedCount $routeCount -MaximumP95 25.0 -Code $Code
        $routeExecutionSum += $routeCount
    }
    if ($ExpectedCache) {
        Assert-FormalCachePair -Cache $Worker.cache -Code $Code
        if ($routeExecutionSum -ne [int64]$Worker.cache.before_close.fts_route.misses) { Throw-Code $Code }
    }
    return [pscustomobject]@{
...
function Assert-FormalPreflightStage {
    ...
    if ($ExpectedSessions -eq 100) {
        foreach ($route in @("full_positive", "exact_active", "category_only", "title_store_exact")) {
            if ([int64]$facts[0].Activation.PSObject.Properties["${route}_route_activation_records"].Value -le 0) {
                $expectedReasons.Add("${route}_activation_zero")
            }
        }
        foreach ($field in @("union_expansion_records", "union_novel_candidate_cells")) {
        ...
        for ($index = 1; $index -le 2; $index++) {
            ...
            foreach ($layer in @("fts_route", "product_view", "mask_decision")) {
                if ([int64]$facts[$index].Cache.before_close.PSObject.Properties[$layer].Value.hits -le 0) {
                    $expectedReasons.Add("${modeName}_${layer}_cache_hits_zero")
                }
            }
        }
    }
...
function Assert-FormalTargetReceipt {
    ...
    if ($Mode -eq "preflight") {
        ...
        if (
            ...
            [string]$Receipt.device.reason -cne "frozen sparse FTS/mask/Fraction-RRF backend"
...
try {
    ...
    try {
        ...
        $runtime = $null
        $complete = $outerIsRawEnvelope -and -not $processCapture.TimedOut -and $processCapture.ExitCode -eq 0 -and [int]$parsedOuter.target_exit_code -eq 0 -and $null -ne $parsedOuter.target_receipt -and $runtimeCleanupPassed -and $formalReceiptValidated
        $terminal = [ordered]@{
            implementation_commit = $ImplementationCommit
            mode = $Mode
